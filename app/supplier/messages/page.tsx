...
const INITIAL_MSGS = [
  { id: "1", from: "FindIT", initials: "FI", text: "Hej Mette! Vi har modtaget dit tilbud på Senior Azure-arkitekten. Kunden kigger på det nu og vi forventer svar inden for 2 arbejdsdage.", time: "I dag 08:30", isOwn: false },
  { id: "2", from: "FindIT", initials: "FI", text: "Har I mulighed for at sende et opdateret CV for Mette Paulsen? Kunden efterspørger detaljer om Azure-certifikationer.", time: "I dag 11:15", isOwn: false },
];
  ...
  return (
    <PortalLayout user={MOCK_SUPPLIER} navItems={NAV} portalType="supplier">
      <div className="mb-6">
        ...
        <p className="text-charcoal/50 text-sm mt-1">Direkte kommunikation med FindIT-teamet</p>
      </div>
      <div className="bg-white rounded-2xl border border-[#ede9e3] flex flex-col overflow-hidden" style={{ height: "calc(100vh - 220px)" }}>
        <div className="px-6 py-4 border-b border-[#f0ede8] flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-charcoal text-sm">FindIT-teamet</div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map(msg => (
            <div key={msg.id} className={`flex gap-3 ${msg.isOwn ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ${msg.isOwn ? "bg-gradient-to-br from-orange to-orange-dark text-white" : "bg-[#2d2c2c] text-white"}`}>{msg.initials}</div>
